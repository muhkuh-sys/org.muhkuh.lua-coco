...
for strMatch in glob.iglob(os.path.join(strCwd, 'lua5.1-coco-*.xml')):
    os.remove(strMatch)
...
astrMatch = glob.glob(os.path.join(strCwd, 'lua5.1-coco-*.xml'))
if len(astrMatch) != 1:
    raise Exception('No match found for "lua5.1-coco-*.xml".')
...
astrCMAKE_COMPILER.append('-DEXTERNAL_LIBUSB_ROOT_DIR=%s' % os.path.join(strCfg_workingFolder, 'external', 'libusb', 'install'))
astrCMAKE_COMPILER.append('-DEXTERNAL_LIBFTDI_ROOT_DIR=%s' % os.path.join(strCfg_workingFolder, 'external', 'libftdi', 'install', 'usr', 'local'))
...
for strMatch in glob.iglob(os.path.join(strCwd, 'lua5.4-coco-*.xml')):
    os.remove(strMatch)
...
astrMatch = glob.glob(os.path.join(strCwd, 'lua5.4-coco-*.xml'))
if len(astrMatch) != 1:
    raise Exception('No match found for "lua5.4-coco-*.xml".')
